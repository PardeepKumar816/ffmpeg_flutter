class Constants {
  Constants._();
  static const String BASE_PATH = '/storage/emulated/0/Download/';

  static const String IMAGE_PATH = BASE_PATH + 'Image01.jpg';
  static const String VIDEO_PATH = BASE_PATH + 'Video.mp4';
  static const String GIF_PATH = BASE_PATH + 'giphy.gif';
  static const String AUDIO_PATH = BASE_PATH + 'Audio.mp3';
  static const String IMAGES_PATH = BASE_PATH + '%02d.jpg';
  static const String OUTPUT_PATH = BASE_PATH + 'output.mp4';
  static const String videoPath = BASE_PATH + 'Video2.mp4';
}
